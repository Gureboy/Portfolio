...
class NeonDBClient {
  ...
  async createCharacter(characterData) {
    try {
      if (!this.playerId) throw new Error('No player logged in');
  ...
  async saveInventory(items) {
    ...
    return await this.apiCall(`/characters/${this.currentCharacter}/inventory`, 'PUT', { items });
  }
  ...
  async saveMaterials(materials) {
    if (!this.currentCharacter) return;
    return await this.apiCall(`/characters/${this.currentCharacter}/materials`, 'PUT', { materials });
  }
  ...
  // Auto-save functionality — pass gameState as second argument
  startAutoSave(character, gameState, interval = 30000) {
    ...
    this.autoSaveInterval = setInterval(async () => {
      ...
      try {
        await this.saveCharacterData({
          ...
          current_encounter: gameState?.currentEncounter || 0,
